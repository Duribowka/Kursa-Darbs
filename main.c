...
int main(int argc, char **argv){
    ...
    for (int i = 1; i < argc; i++) {
        ...
        else if (mystrcmp(argv[i], "-price") == 0){
            if (i + 1 >= argc){
                continue;
            }
            s->price = myatoi(argv[i+1]);
        }
        else if (mystrcmp(argv[i], "-weight") == 0){
            if (i + 1 >= argc){
                continue;
            }
            s->weight = myatoi(argv[i+1]);
        }
        else if (mystrcmp(argv[i], "-height") == 0){
            if (i + 1 >= argc){
                continue;
            }
            s->height = myatoi(argv[i+1]);
        }
        else if (mystrcmp(argv[i], "-width") == 0){
            if (i + 1 >= argc){
                continue;
            }
            s->width = myatoi(argv[i+1]);
        }
        else if (mystrcmp(argv[i], "-stock") == 0){
            if (i + 1 >= argc){
                continue;
            }
            s->stock = myatoi(argv[i+1]);
        }
        ...
        else if (mystrcmp(argv[i], "--filter+sort") == 0) {
            if (i + 2 >= argc) {
                printf("Usage: --filter+sort <genre> <mode>\n");
                continue;
            }
            if (!isAuthor)
                filter_and_sort_items(argv[i + 1], argv[i + 2]);
            else
                filter_and_sort_authors(argv[i + 1], argv[i + 2]);
